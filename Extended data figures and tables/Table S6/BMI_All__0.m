...
% 按照每一个国家、每一次实验确定系数a和b
% 针对所有国家数据，计算BMI(Weight/Height^2)
% 将数据分为 Age < 25, 25<= Age < 50 and Age >= 50 三个组别
%
tic;
%%
c = 1;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Female_All__50 = [BMI_Female_All__50 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 51;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Male_All__50 = [BMI_Male_All__50 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 51;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Female_All__25 = [BMI_Female_All__25 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 3;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Female_All__25_50 = [BMI_Female_All__25_50 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 26;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Male_All__25 = [BMI_Male_All__25 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 3;
...
[M N] = size(X_Temp);
for i = 1:M
    BMI_Male_All__25_50 = [BMI_Male_All__25_50 Y_Temp(i)/X_Temp(i)^2]; % Weight/Height^2
end
Age = 26;
...
% we Calculate the BMI distribution when c = 1; in Weight/Height^2^c
% For Female
Age = 50;
X_Temp = Female_data_50(:,1)/100; % Height (m)
Y_Temp = Female_data_50(:,2); % Weight (Kg)
...
Text = text(1.13,0.8*max(yy),Text,'FontSize',12,'FontName',My_FontName,'HorizontalAlignment','right');
