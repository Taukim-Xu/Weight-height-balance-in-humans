%% Source codes for dHr_Hp_03_KNHANES.m
tic;
%%
c = 1;
...
dHr_Hp_03_KNHANES_Female = []; % Wr - Wp: The real weight minus the weight predicted by the model
dHr_Hp_03_KNHANES_Male = []; % Wr - Wp: The real weight minus the weight predicted by the model
...
[M N] = size(X_Temp);
for i = 1:M
    dHr_Hp_03_KNHANES_Female = [dHr_Hp_03_KNHANES_Female X_Temp(i) - (Y_Temp(i)-Coefficient_a)/(Coefficient_b*Y_Temp(i))]; % Hr - Hp
end
% Age = 2;
...
[M N] = size(X_Temp);
for i = 1:M
    dHr_Hp_03_KNHANES_Male = [dHr_Hp_03_KNHANES_Male X_Temp(i) - (Y_Temp(i)-Coefficient_a)/(Coefficient_b*Y_Temp(i))]; % Hr - Hp
end
% Age = 2;
...
toc;
%% We save the data
save dHr_Hp_03_KNHANES.mat dHr_Hp_03_KNHANES_Female dHr_Hp_03_KNHANES_Male
