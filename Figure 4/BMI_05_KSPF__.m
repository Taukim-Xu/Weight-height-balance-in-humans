%% Source codes for BMI_05_KSPF__.mat
% 
%
tic;
%%
...
BMI_Female_05_KSPF__ = [];
BMI_Male_05_KSPF__ = [];
% For Female
[M N] = size(Female_data); % Age Height(cm) Weight(Kg)
for i = 1:M
    BMI_Female_05_KSPF__ = [BMI_Female_05_KSPF__ Female_data(i,3)/(Female_data(i,2)/100)^2];
end

% For Male
[M N] = size(Male_data); % Age Height(cm) Weight(Kg)
for i = 1:M
    BMI_Male_05_KSPF__ = [BMI_Male_05_KSPF__ Male_data(i,3)/(Male_data(i,2)/100)^2];
end
toc;

%% We save the data
save BMI_05_KSPF__.mat BMI_Female_05_KSPF__ BMI_Male_05_KSPF__
