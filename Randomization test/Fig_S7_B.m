...
for i = 1:M
    ...
    Height_random(i,1) = Height_pool(No,1);
end
...
figure(FF)
xx = HH_Raw.BinEdges;
yy = HH_Raw.Values/max(HH_Raw.Values);
[M N] = size(HH_Raw.BinEdges);
for i = 1:N-1
...
My_P = prctile(sWHI_raw,My_Percentile);
[M N] = size(HH_Raw.BinEdges);
x_data = [];
y_data = HH_Raw.Values;
for i = 1:N-1
...
% 5th and 95th
Prctile_Female = prctile(sWHI_raw,My_Percentile);
...
Std_01 = var(sWHI_raw); % Variance for raw dataset
My_str_01 = ['Variance = ' num2str(round(Std_01,6))];
text(0.9,1.1,My_str_01,'FontName',My_FontName,'FontSize',My_fontSize)
...
figure(FF)
xx = HH_Random.BinEdges;
yy = HH_Random.Values/max(HH_Random.Values);
[M N] = size(HH_Random.BinEdges);
for i = 1:N-1
    X_Fill = [xx(i) xx(i+1) xx(i+1) xx(i)];
    Y_Fill = [0 0 yy(i) yy(i)];
    H = fill(X_Fill,Y_Fill,My_Colors_4_1(2,:));
...
My_P = prctile(sWHI_random,My_Percentile);
[M N] = size(HH_Random.BinEdges);
x_data = [];
y_data = HH_Random.Values;
for i = 1:N-1
    x_data = [x_data (xx(i)+xx(i+1))/2];
end
[fitobject,gof] = fit(x_data',y_data','smooth');
...
% 5th and 95th
Prctile_Female = prctile(sWHI_random,My_Percentile);
XX = [Prctile_Female(1) Prctile_Female(1)]; 
YY = [0 1.2]; 
HH1 = plot(XX,YY,':','LineWidth',My_LineWidth-0.5,'Color',My_Colors_4_1(2,:)); % 5th
XX = [Prctile_Female(3) Prctile_Female(3)]; 
...
Std_02 = var(sWHI_random); % Variance for random dataset
My_str_02 = ['Variance = ' num2str(round(Std_02,6))];
text(0.9,1.1,My_str_02,'FontName',My_FontName,'FontSize',My_fontSize)
